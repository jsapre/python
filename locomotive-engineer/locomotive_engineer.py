...
def fix_list_of_wagons(each_wagons_id: list, missing_wagons: list):
    ...
    :return: list - list of wagons.
    """
    result = each_wagons_id[2:] + each_wagons_id[0:2]
    ind = result.index(1)
    return result[0 : ind + 1] + missing_wagons + result[ind + 1 :]
...
def fix_wagon_depot(wagons_rows: list):
    ...
    """
    row1, row2, row3 = zip(wagons_rows[0], wagons_rows[1], wagons_rows[2])
